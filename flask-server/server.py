...
def get_ai_message():
    try:
        ...
        stream = client.chat.completions.create(
            model="ft:gpt-4o-mini-2024-07-18:personal:pandas-expert:A6aow7uo",
            messages=[
        ...
        print(ai_response)
        print(audi_vw_data.shape)
        query = ai_response.split('Query: ')[-1].strip()
        variable_name, expression = query.split('=', 1)
        expression = expression.strip().replace('df', 'audi_vw_data')
        ...
        gpt4o_mini_response = client.chat.completions.create(
          model="gpt-4o-mini",
          messages=[
              {"role": "system", "content": "You are a helpful assistant helping users with car related problems and questions. You do not know anything beyond that scope."},
              {"role": "system", "content": f"The user has retrieved data from a database about the asked queastion. Based on the provided data: {query_result_str}"},
...
def train_model(model_choice):
    ...
    for brand, data in data_dict.items():
        ...
        os.makedirs(f'flask-server/{brand}_models', exist_ok=True)
        
        # Define paths for current model and backup folder
        model_filename_prefix = f'{model_choice}_model'
        model_path_pattern = f'flask-server/{brand}_models/{model_filename_prefix}_*.joblib'
        backup_folder = f'flask-server/old_{brand}_models'
        ...
        new_model_path = f'flask-server/{brand}_models/{model_choice}_model_{timestamp}.joblib'
...
def get_models():
    ...
    if model_type == 'new':
        folder_to_search = f'flask-server/{brand}_models'
    elif model_type == 'old':
        folder_to_search = f'flask-server/old_{brand}_models'
...
def set_selected_model():
    ...
    if model_type == 'new':
        folder_to_search = f'flask-server/{selected_brand}_models'
    elif model_type == 'old':
        folder_to_search = f'flask-server/old_{selected_brand}_models'
...
def get_prediction():
    ...
    try:
        ...
        filtered_vehicles = model_data[
            ...
            (model_data['year'] <= int(year_value) + 3) &
            (model_data['price'] >= prediction - (prediction*0.20)) &
            (model_data['price'] <= prediction + (prediction*0.20))
        ]

        if 'kilowatts' in data and data['kilowatts'] not in [None, ""]:
            percentage = get_value('kilowatts') * 0.15
            kw_low = get_value('kilowatts') - percentage
            kw_high = get_value('kilowatts') + percentage
